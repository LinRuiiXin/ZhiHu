...
public class ComprehensiveFragment extends SearchFragment {

    private View view;
    private Activity activity;
    private Keyword keyword;
    private final List<Information> data;
    private SearchComprehensiveAdapter searchComprehensiveAdapter;
    private LinearLayoutManager linearLayoutManager;
    private String serverLocation;
    private final Gson gson;

    public ComprehensiveFragment(Keyword keyword) {
        super(keyword);
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if(view == null){
            view = inflater.inflate(R.layout.search_comprehensive_fragment,container,false);
            init();
        }
        return view;
    }
    ...
    private void init() {
        SmartRefreshLayout refreshLayout = view.findViewById(R.id.scf_refresh_layout);
        refreshLayout.setOnRefreshListener(this);
        refreshLayout.setOnLoadMoreListener(this);
        refreshLayout.autoRefresh();
        RecyclerView content = view.findViewById(R.id.scf_content);
        content.setAdapter(searchComprehensiveAdapter);
    ...
    void refresh(int size, Consumer<Boolean> isSuccess, Consumer<Integer> len) {
        data.clear();
        searchComprehensiveAdapter.notifyDataSetChanged();
        loadMore(0,size,isSuccess,len);
    }

    @Override
    void loadMore(int limit, int size, Consumer<Boolean> isSuccess, Consumer<Integer> len) {
        String url = serverLocation + "/SearchService/Search/Comprehensive/" + keyword.getTitle().trim() + "/" + limit + "/" + size;
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                activity.runOnUiThread(()->{
                    if(simpleDto.isSuccess()){
                        List<Information> res = gson.fromJson(gson.toJson(simpleDto.getObject()),new TypeToken<List<Information>>(){}.getType());
                        if(res != null){
                            data.addAll(res);
                            len.accept(res.size());
                        }
